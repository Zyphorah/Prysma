...
* **Le code source Prysma** minimal permettant de reproduire l'error.
* **L'environnement d'exécution** (Version de LLVM, OS).
* **Les traces de diagnostic :** Si le crash survient à la compilation, fournissez la trace LLVM IR. Si c'est une error mémoire, fournissez la output de l'AddressSanitizer (ASAN).

## 2. Standards de Développement (C++)

Le cœur du compiler est écrit en C++17. Les règles suivantes sont non négociables :

* **Séparation des préoccupations :** Respectez le découplage strict input le Frontend (Lexer/Parser) et le Backend (LLVM). 
...
1. Créez une branche dérivée (*fork*) et travaillez sur une branche functionnelle (`feat/nom-functionnalite` ou `fix/nom-bug`).
