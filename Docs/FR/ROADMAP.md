...
Ce document détaille les capacités actuelles du langage Prysma ainsi que les axes de développement futurs. Le projet est structuré autour d'une architecture modulaire utilisant LLVM comme backend de compilation.

## 1. État actuel du projet (Stable)
...
* **Structures de Contrôle :**
    ...
    * Boucles `while` avec gestion de la pile via `alloca`.
* **Fonctions :** Support des fonctions récursives, passage d'arguments par valeur et gestion des types de retour.
* **Gestion des Identifiants :** Déclaration de variables, affectations et résolution de portée (scoping).
...
* **Allocation Dynamique :** Intégration de `prysma_malloc` et `free` issus de la bibliothèque standard C.
...
* **Visualisation :** Export de l'Arbre Syntaxique Abstrait (AST) aux formats texte et graphique (Graphviz/DOT).
...
* **Built-in Functions :** Intégration de fonctions mathématiques avancées (puissance, PGCD) au niveau du runtime.
...
* **Generics :** Support de la programmation générique (templates) pour les fonctions et les classes.
